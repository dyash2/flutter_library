...
export default function Home() {
  ...
  // ── Dynamic categories — derived from actual DB records ──────
  // Sorted alphabetically, always in sync with what's in the data.
  const liveCategories = useMemo(() => {
    const cats = [...new Set(questions.map(q => q.category).filter(Boolean))];
    return cats.sort((a, b) => a.localeCompare(b));
  }, [questions]);

  // For the filter chips: 'All' + every distinct category in DB
  const filterCategories = ['All', ...liveCategories];

  // If the active category filter no longer exists (e.g. last question deleted),
  // reset back to All so the user isn't stuck on an empty view
  useEffect(() => {
  ...
  return (
   ...
   <div className="max-w-6xl mx-auto flex items-center justify-between px-4">
     ...
     <p className="flex items-center gap-1">
       ...
       <a         href="https://dyash2.github.io/portfolio/"
         target="_blank"
     ...
     >
      ...
      {showForm && <QuestionForm onSubmit={handleAdd} onClose={() => setShowForm(false)} categories={liveCategories} />}
      {editItem  && <QuestionForm initial={editItem} onSubmit={handleEdit} onClose={() => setEditItem(null)} categories={liveCategories} />}
    </>
  );
}